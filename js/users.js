    ...
    return `<tr>
      <td>
        <div style="display:flex;align-items:center;gap:10px">
          ...
          <div>
            <div class="text-bold" style="font-size:13px">${esc(u.full_name||'')}${isMe ? ' <span style="font-size:10px;background:var(--blue-100);color:var(--blue-700);padding:1px 6px;border-radius:8px">คุณ</span>' : ''}</div>
      ...
      <td>
        <div style="display:flex;gap:5px;justify-content:flex-end">
          ...
          ${!isMe ? `<button class="btn btn-xs ${u.is_active ? 'btn-danger' : 'btn-secondary'}" onclick="toggleUserActive('${u.id}',${!u.is_active})">
            <i class="ti ${u.is_active ? 'ti-user-off' : 'ti-user-check'}"></i>${u.is_active ? 'ปิด' : 'เปิด'}
          </button>` : ''}
        </div>
      </td>
    </tr>`;
...
function openAddUser() {
  ...
  ['uform-fullname','uform-email','uform-password','uform-username','uform-phone'].forEach(id => {
...
function openEditUser(uid) {
  ...
  document.getElementById('uform-pass-row').style.display = 'none'; // ไม่แสดง password ตอนแก้ไข
...
async function saveUser() {
  ...
  if (_editUserId) {
    ...
    if (!password || password.length < 6) { reset(); toast('รหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร', 'error'); return; }

    // ใช้ client แยก (ไม่กระทบ session ปัจจุบัน)
    const { data, error } = await _sbCreate.auth.signUp({
      email, password,
      options: { data: { full_name: fullName, username, phone, role } },
    });
    if (error) { reset(); toast('สร้างผู้ใช้ล้มเหลว: ' + error.message, 'error'); return; }

    // อัปเดต profile เพิ่มเติม
    if (data.user) {
      await _sb.rpc('admin_update_user', {
...
async function toggleUserActive(uid, newActive) {
  const u = _allUsers.find(x => x.id === uid); if (!u) return;
  const { error } = await _sb.rpc('admin_update_user', {
    p_user_id:   uid,
