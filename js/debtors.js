...
function renderDebtors() {
  ...
  // Group by member — sum totals, track oldest due date
  const grouped = {};
  overdue.forEach(b => {
...
function _updateDebtorKpis(grouped) {
  ...
  const totalDebt = all.reduce((s, g) => s + g.total, 0);
  ...
  if (kpis[3]) kpis[3].textContent = '฿' + totalDebt.toLocaleString();
}

// ─── Debtor Actions ───
function suspendMember(mid) {
...
function notifyAllDebtors() {
  const mids = [...new Set(bills.filter(b => b.status === 'overdue').map(b => b.memberId))];
...
function suspendAllOverdue3Months() {
  const today = new Date();
  const grouped = {};
