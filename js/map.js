...
let _mapMarkers   = [];
let _mapZones     = [];
let _mapAllShown  = true;

// Village center coordinates (fallback when member has no GPS)
...
function _effectiveLatLng(m) {
  ...
  // Deterministic pseudo-random offset so markers don't overlap
  const seed = (m.id * 9301 + 49297) % 233280;
  const rnd  = seed / 233280;
  const seed2 = (m.id * 1234 + 5678) % 9999;
  const rnd2  = seed2 / 9999;
  return [center[0] + (rnd - 0.5) * 0.003, center[1] + (rnd2 - 0.5) * 0.003];
}
...
function renderMap() {
  ...
  // Leaflet not loaded yet (CDN still fetching)
  if (typeof L === 'undefined') {
    container.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--gray-500)">
      <div style="text-align:center;padding:32px">
        <i class="ti ti-map-off" style="font-size:48px;display:block;margin-bottom:12px"></i>
        <div style="font-size:14px;font-weight:600">กำลังโหลดไลบรารีแผนที่...</div>
        <div style="font-size:12px;margin-top:8px">กรุณารอสักครู่แล้วกดรีเฟรช</div>
      </div></div>`;
  ...
  if (!members.length) {
    container.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--gray-500)">
      <div style="text-align:center;padding:32px">
        <i class="ti ti-users-group" style="font-size:48px;display:block;margin-bottom:12px"></i>
        <div style="font-size:14px;font-weight:600">ยังไม่มีข้อมูลสมาชิก</div>
        <div style="font-size:12px;margin-top:8px">กรุณาเพิ่มสมาชิกหรือตรวจสอบการเชื่อมต่อ Supabase</div>
      </div></div>`;
    return;
  }
  ...
  // Use real GPS when available, otherwise village-center fallback
  ...
  Object.entries(vGroups).forEach(([v, g]) => {
    ...
    const zone  = L.circle([cLat, cLng], {
      radius: 120, color, fillColor: color,
      fillOpacity: 0.1, weight: 2, dashArray: '7 5',
  ...
  members.forEach(m => {
    ...
    const marker = L.circleMarker([lat, lng], {
      radius, color: '#fff', weight: 2, fillColor: fill, fillOpacity: 0.9,
    });

    const debtHtml = totalDebt > 0
      ? `<div style="color:#dc2626;font-weight:700;font-size:12px;margin-top:4px">ค้างชำระ ฿${totalDebt.toLocaleString()}</div>`
      : '';

    marker.bindPopup(`
      <div style="font-family:'Sarabun',sans-serif;min-width:200px;padding:2px">
        ...
        <div style="display:flex;gap:6px;margin-top:10px">
          <button onclick="closeMemberMapPopup();openMemberQuickView(${m.id})"
            style="flex:1;padding:5px;background:#00236f;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-family:'Sarabun',sans-serif;font-weight:600">
            <span>&#128196;</span> ดูบิล
          </button>
          <button onclick="closeMemberMapPopup();gotoMeter('${esc(m.firstName + ' ' + m.lastName)}','${esc(m.meter)}',${m.id})"
            style="flex:1;padding:5px;background:#0ea5e9;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-family:'Sarabun',sans-serif;font-weight:600">
            <span>&#128338;</span> จดมิเตอร์
          </button>
        </div>
      </div>`, { maxWidth: 240 });
...
function closeMemberMapPopup() {
  if (_mapInstance) _mapInstance.closePopup();
}

function filterMapByVillage(village) {
  if (!_mapInstance) return;
  _mapMarkers.forEach(({ marker, village: v }) => {
    if (!village || v === village) marker.addTo(_mapInstance);
    else marker.remove();
  });
}
...
function filterMapByStatus(status) {
  ...
  _mapMarkers.forEach(({ marker, status: s }) => {
    if (!status || s === status) marker.addTo(_mapInstance);
    else marker.remove();
  });
}
